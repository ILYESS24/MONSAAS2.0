...
const MOCK_VALUES: ClerkSafeReturn = {
  openSignIn: () => {
    console.warn('Clerk not configured - Sign in unavailable');
  },
  openSignUp: () => {
    console.warn('Clerk not configured - Sign up unavailable');
  },
  user: null,
  signOut: () => {
    console.warn('Clerk not configured - Sign out unavailable');
  },
  getToken: async () => null,
  loaded: true,
  session: null,
