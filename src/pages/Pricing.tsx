...
const PricingCard = ({ planId, isYearly, isPopular, onSubscribe, isLoading, loadingPlan }: PricingCardProps) => {
  ...
  return (
    <div className={cn(
      ...
      <div className="mb-8">
        <h3 className={cn("text-xl font-bold mb-4", isEnterprise ? "text-white" : "text-gray-900")}>
          {plan.name}
        </h3>
        ...
        <p className={cn("text-sm leading-relaxed", isEnterprise ? "text-gray-400" : "text-gray-500")}>
      ...
      <button 
        ...
        disabled={isLoading}
        className={cn(
          "w-full py-4 rounded-xl text-center font-bold text-sm transition-all border flex items-center justify-center gap-2",
          isEnterprise 
...
export default function Pricing() {
  ...
  const handleSubscribe = async (planId: PlanId) => {
    ...
    try {
      await redirectToCheckout(planId, user?.primaryEmailAddress?.emailAddress);
    } catch (error: unknown) {
