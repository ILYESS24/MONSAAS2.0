...
export default function DashboardLayout() {
  ...
  return (
    <div className="h-screen w-screen bg-[#F8F9FB] font-sans flex flex-col overflow-hidden relative">
      ...
      <header className="h-[60px] md:h-[80px] px-4 md:px-8 flex items-center justify-between flex-shrink-0 bg-[#F8F9FB] z-20">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate('/')}
            className="text-lg md:text-2xl font-bold text-gray-900 tracking-tight hidden sm:block hover:text-primary transition-colors cursor-pointer"
          >
            AURION
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-black text-white flex items-center justify-center font-bold text-sm shadow-md cursor-pointer hover:scale-105 transition-transform">
            {user?.firstName?.[0] || "U"}
          </div>
        </div>
      </header>
