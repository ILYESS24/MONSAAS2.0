    ...
    <button
      onClick={(e) => {
        ...
        if (typeof onUpgrade === 'function') {
          console.log('✅ onUpgrade is a function');
          onUpgrade();
        } else {
          console.error('❌ onUpgrade is not a function:', onUpgrade);
        }
      }}
...
export default function DashboardStudio() {
  ...
  const handlePlanUpgrade = async (plan: PlanType) => {
    ...
    try {
      ...
      console.log('🔄 Converting plan', plan, 'to Stripe plan ID:', stripePlanId);
      ...
      console.log('📦 Checkout result received:', checkoutResult);

      if (checkoutResult && checkoutResult.success && checkoutResult.url) {
        console.log('✅ Redirecting to Stripe:', checkoutResult.url);
      ...
      console.error('❌ Unexpected error in handlePlanUpgrade:', error);
      toast({
        title: "Erreur",
  ...
  const toolRoutes: Record<ToolType, string> = {
    image_generation: '/creation/image',
    video_generation: '/creation/video',
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500 pb-10">
      ...
      {activeTab === "History" && (
        <Card>
          <h3 className="font-bold text-lg mb-6">Historique Complet</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left text-xs text-gray-400 border-b border-gray-100">
                  ...
                  <th className="pb-3 font-medium">Crédits</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={log.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                      <td className="py-3">
                        ...
                        <p className="text-xs text-gray-400">{date.toLocaleTimeString('fr-FR')}</p>
                      </td>
                      <td className="py-3">{TOOL_LABELS[log.tool]}</td>
                      <td className="py-3 font-bold">-{log.credits}</td>
                    </tr>
                  );
                })}
